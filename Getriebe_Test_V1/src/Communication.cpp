...
uint8_t (*Communication::receiveMessage(bool &outReceivedMessage))[32]
{
    while (Wire.available() > 0)
    {
        messageBuffer[messageBufferIndex] = Wire.read();
        messageBufferIndex++;
    }
    if (messageBufferIndex == 31)
    {
        messageBufferIndex = 0;
        outReceivedMessage = true;
        // Return shared pointer to messageBuffer
        return &messageBuffer;
    }
    else
    {
        outReceivedMessage = false;
        return nullptr;
    }
}
